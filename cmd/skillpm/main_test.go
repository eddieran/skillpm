...
func TestRemoveHasAliases(t *testing.T) {
	...
	for _, alias := range []string{"detach", "eject"} {
...
func TestScheduleListHasLsAlias(t *testing.T) {
	...
	for _, c := range scheduleCmd.Commands() {
		if c.Name() == "list" {
			for _, alias := range []string{"ls", "status", "st", "show", "get", "info"} {
				if !containsString(c.Aliases, alias) {
...
func TestScheduleInstallHasAddAlias(t *testing.T) {
	...
	for _, c := range scheduleCmd.Commands() {
		if c.Name() == "install" {
			for _, alias := range []string{"add", "create", "on", "enable", "set", "start", "update", "resume", "up"} {
				if !containsString(c.Aliases, alias) {
