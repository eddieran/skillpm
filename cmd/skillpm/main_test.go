...
func TestScheduleHasSchedAlias(t *testing.T) {
	...
	for _, alias := range []string{"sched", "sch", "scheduler", "cron"} {
...
func TestScheduleListHasLsAlias(t *testing.T) {
	...
	for _, c := range scheduleCmd.Commands() {
		if c.Name() == "list" {
			for _, alias := range []string{"ls", "status", "st", "show", "get"} {
				if !containsString(c.Aliases, alias) {
...
func TestScheduleInstallHasAddAlias(t *testing.T) {
	...
	for _, c := range scheduleCmd.Commands() {
		if c.Name() == "install" {
			for _, alias := range []string{"add", "on", "enable", "set", "start", "update"} {
				if !containsString(c.Aliases, alias) {
...
func TestScheduleRemoveHasRmAlias(t *testing.T) {
	...
	for _, c := range scheduleCmd.Commands() {
		if c.Name() == "remove" {
			for _, alias := range []string{"rm", "off", "disable", "stop", "del", "delete", "uninstall"} {
				if !containsString(c.Aliases, alias) {
