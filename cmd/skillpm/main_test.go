...
func TestSelfStableShortcutHasAliases(t *testing.T) {
	...
	for _, alias := range []string{"selfstable", "stable-selfpm", "stable-self", "self-release"} {
		if !containsString(cmd.Aliases, alias) {
...
func TestSelfStableShortcutAliasesResolveThroughRootCommand(t *testing.T) {
	root := newRootCmd()
	for _, alias := range []string{"self-stable", "selfstable", "stable-selfpm", "stable-self", "self-release"} {
		resolved, _, err := root.Find([]string{alias})
		if err != nil {
			t.Fatalf("find %s failed: %v", alias, err)
		}
...
func TestSelfEdgeShortcutHasAliases(t *testing.T) {
	...
	for _, alias := range []string{"selfedge", "edge-selfpm", "edge-self", "self-nightly"} {
		if !containsString(cmd.Aliases, alias) {
...
func TestSelfEdgeShortcutAliasesResolveThroughRootCommand(t *testing.T) {
	root := newRootCmd()
	for _, alias := range []string{"self-edge", "selfedge", "edge-selfpm", "edge-self", "self-nightly"} {
		resolved, _, err := root.Find([]string{alias})
		if err != nil {
			t.Fatalf("find %s failed: %v", alias, err)
		}
...
func TestSelfBetaShortcutHasAliases(t *testing.T) {
	...
	for _, alias := range []string{"selfbeta", "beta-selfpm", "beta-self", "self-preview", "preview-self", "beta-preview", "preview-selfpm"} {
		if !containsString(cmd.Aliases, alias) {
...
func TestSelfBetaShortcutAliasesResolveThroughRootCommand(t *testing.T) {
	root := newRootCmd()
	for _, alias := range []string{"self-beta", "selfbeta", "beta-selfpm", "beta-self", "self-preview", "preview-self", "beta-preview", "preview-selfpm"} {
		resolved, _, err := root.Find([]string{alias})
		if err != nil {
			t.Fatalf("find %s failed: %v", alias, err)
		}
