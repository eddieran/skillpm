...
func TestSelfBetaShortcutHasAliases(t *testing.T) {
	...
	for _, alias := range []string{"selfbeta", "beta-selfpm", "beta-self", "self-preview", "preview-self", "beta-preview"} {
		if !containsString(cmd.Aliases, alias) {
...
func TestSelfBetaShortcutAliasesResolveThroughRootCommand(t *testing.T) {
	root := newRootCmd()
	for _, alias := range []string{"self-beta", "selfbeta", "beta-selfpm", "beta-self", "self-preview", "preview-self", "beta-preview"} {
		resolved, _, err := root.Find([]string{alias})
		if err != nil {
			t.Fatalf("find %s failed: %v", alias, err)
		}
