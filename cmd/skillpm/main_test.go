...
func TestSelfStableShortcutHasAliases(t *testing.T) {
	...
	for _, alias := range []string{"selfstable", "stable-selfpm", "stable-self", "self-release", "release-selfpm", "self-ga", "ga-selfpm"} {
		if !containsString(cmd.Aliases, alias) {
...
func TestSelfStableShortcutAliasesResolveThroughRootCommand(t *testing.T) {
	root := newRootCmd()
	for _, alias := range []string{"self-stable", "selfstable", "stable-selfpm", "stable-self", "self-release", "release-selfpm", "self-ga", "ga-selfpm"} {
		resolved, _, err := root.Find([]string{alias})
		if err != nil {
			t.Fatalf("find %s failed: %v", alias, err)
		}
...
func TestSelfEdgeShortcutHasAliases(t *testing.T) {
	...
	for _, alias := range []string{"selfedge", "edge-selfpm", "edge-self", "self-nightly", "nightly-selfpm", "self-canary", "canary-selfpm", "self-dev", "dev-selfpm"} {
		if !containsString(cmd.Aliases, alias) {
...
func TestSelfEdgeShortcutAliasesResolveThroughRootCommand(t *testing.T) {
	root := newRootCmd()
	for _, alias := range []string{"self-edge", "selfedge", "edge-selfpm", "edge-self", "self-nightly", "nightly-selfpm", "self-canary", "canary-selfpm", "self-dev", "dev-selfpm"} {
		resolved, _, err := root.Find([]string{alias})
		if err != nil {
			t.Fatalf("find %s failed: %v", alias, err)
		}
